...
def test_default_catalog_is_sqlite(fdl_project_dir: Path):
    """fdl init creates a SQLite catalog (v0.9 local catalogs are always SQLite)."""
    import sqlite3
...
def test_double_init_preserves_existing_files(fdl_project_dir: Path):
    ...
    assert (fdl_project_dir / ".fdl" / "default" / "ducklake.sqlite").exists()
