...
def test_convert_rejects_non_main_metadata_schema(tmp_path: Path):
    """A source whose ducklake_* metadata lives outside 'main' (METADATA_SCHEMA)
    is rejected with a clear error instead of silently producing an empty
    catalog."""
    ...
    conn.execute("INSERT INTO lake.main.t VALUES (1)")
    conn.execute("DETACH lake")
    conn.close()

    with pytest.raises(ValueError, match="custom_meta"):
        _convert_ducklake_catalog(
            src,
            dst,
            src_type="duckdb",
